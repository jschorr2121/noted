...
const MAX_CHUNK_MB = 3.5; // Stay well under Vercel's ~4.5MB limit
...
export default function Home() {
  ...
  // Get duration of a file loaded in ffmpeg
  const getDuration = async (ffmpeg: FFmpeg, filename: string): Promise<number> => {
    ...
    // Run a quick probe by trying to convert 0 seconds
    await ffmpeg.exec(["-i", filename, "-f", "null", "-t", "0", "/dev/null"]).catch(() => {});
    ffmpeg.off("log", handler);
    return duration;
  };

  // Compress and chunk audio into uploadable pieces
  const compressAndChunk = async (inputFile: File): Promise<File[]> => {
    ...
    // At 64kbps mono, 1 minute ≈ 0.48MB. So ~7 minutes per 3.5MB chunk.
    const minutesPerChunk = (MAX_CHUNK_MB * 8) / 0.064 / 60; // ~7.3 minutes
  ...
  const processFile = async () => {
    ...
    try {
      ...
      try {
        ...
        // Fallback: try uploading original if small enough
        ...
        <div className={`flex-1 px-6 py-8 transition-all ${showLogs ? "pr-0" : ""}`}>
          ...
          {stage === "idle" && !notes && (
            <div className="flex flex-col items-center justify-center min-h-[60vh]">
              ...
              {file && (
                <button onClick={processFile} className="mt-6 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors">
                  Generate Notes
                </button>
              )}
            </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="flex gap-1 bg-neutral-900 rounded-lg p-1">
                    <button onClick={() => setActiveTab("notes")} className={`px-4 py-1.5 text-sm rounded-md transition-colors ${activeTab === "notes" ? "bg-neutral-800 text-neutral-100" : "text-neutral-500 hover:text-neutral-300"}`}>Notes</button>
                    <button onClick={() => setActiveTab("transcript")} className={`px-4 py-1.5 text-sm rounded-md transition-colors ${activeTab === "transcript" ? "bg-neutral-800 text-neutral-100" : "text-neutral-500 hover:text-neutral-300"}`}>Transcript</button>
                  </div>
        ...
        {showLogs && (
          <div className="w-80 border-l border-neutral-800/50 flex flex-col bg-neutral-950/80">
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800/50">
              ...
              <button onClick={() => setShowLogs(false)} className="text-neutral-600 hover:text-neutral-400 transition-colors">
